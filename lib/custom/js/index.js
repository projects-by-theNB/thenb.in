(function ($) {
  ...
  // Testimonials carousel is initialised in load-components.js after the
  // async testimonials partial is injected (the markup isn't in the DOM yet
  // at this point).
  ...
  var modulePageMap = {
    ...
    'crm':       'crm.html',
    'custom':    'custom.html'
    // gatepass intentionally absent — falls back to #details-gatepass overlay
  };
  ...
  // Mouse movement effects — delegated so it still binds when the
  // Journey section is injected asynchronously via #journey-placeholder.
  $(document).on('mousemove', '#call-to-action', function(e) {
    const $this = $(this);
