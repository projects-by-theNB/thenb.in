...
var SignupPopup = (function () {
  ...
  /** Return true if the user has already signed up */
  ...
  function show() {
    ...
    if (hasSignedUp()) return;   // never show to users who already signed up
  ...
  function close() {
    ...
    if (wrapper) {
      ...
      setTimeout(function () {
        ...
        if (!state.popupShown && !hasSignedUp()) {
  ...
  function init(configOrClientId) {
    ...
    // If user already signed up, do nothing — ever
    ...
    setTimeout(function () {
      // Re-check right before showing in case another tab signed up meanwhile
      if (hasSignedUp()) return;
      show();
    }, waitMs);
  }
